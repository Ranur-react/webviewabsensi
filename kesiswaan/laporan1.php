        ...
        <div class="widget-content">
          <form method="get">
			...
			<div class="control-group">
              ...
              <select name="mp">
                <?php
                $query_mp=mysqli_query($konek,"SELECT * FROM pelajaran");
                while ($data_mp=mysqli_fetch_array($query_mp)) {
                  echo "<option value='$data_mp[id_pelajaran]'>$data_mp[mata_pelajaran]</option>";
                }
                 ?>
              </select>
            </div>
              ...
              <tbody>
                <tr class="odd gradeX">
                  <?php $bulan=intval($_GET['bulan']-1); ?>
					...
					<td>
						<?php
						$getmp=mysqli_query($konek,"SELECT * FROM pelajaran WHERE id_pelajaran='$_GET[mp]'");
						while ($getm=mysqli_fetch_assoc($getmp)) { ?>
						<?php echo "$getm[mata_pelajaran]"; }?></td>
					<td><a href="report1.php?bulan=<?php echo "$_GET[bulan]&tahun=$_GET[tahun]&kelas=$_GET[kelas]&matapelajaran=$_GET[mp]"; ?>"target="_blank">
