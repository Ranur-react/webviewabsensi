...
SELECT nisn, g.nama as namag,s.nama as namas,mata_pelajaran,jam,tahun_ajar,semester FROM absen_siswa a
					...
					AND jm.id_pelajaran='$_GET[matapelajaran]'
");
...
,COUNT(IF(keterangan ='a',1,NULL))AS alpa FROM absen_siswa a
					...
					WHERE MONTH(a.tgl_absen)='$_GET[bulan]' 
					AND YEAR(a.tgl_absen)='$_GET[tahun]' AND jm.id_kelas='$_GET[kelas]'
					AND jm.id_pelajaran='$_GET[matapelajaran]' GROUP BY namas
");
...
<html>
  ...
  <body>
		...
		<hr>
		<div style="width: 100%;">
			<h3 align="center">Rekam Absensi Per MataPelajaran</h3><br>
			<table class="none">
				<tr>
					<td>MataPelajaran</td> 
					<td>: </td>
					<td><?php echo $header_ta['mata_pelajaran']?></td>
				</tr>
				<tr>
					<td>Kelas</td> 
					<td>: </td>
				...
				<tr>
					<td>Guru</td> 
					<td>: </td>
					<td><?php echo $header_ta['namag']?></td>
				</tr>
			</table>
		</div><br>
